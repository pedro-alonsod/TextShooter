...
class BulletNode: SKNode {
    ...
    @objc var thrust: CGVector = CGVector(dx: 0, dy: 0)
    ...
    @objc class func bullet(from start: CGPoint, toward destination:
        CGPoint) -> BulletNode {
        ...
        bullet.run(SKAction.playSoundFileNamed("bullet.wav", waitForCompletion: false))
        
        return bullet
    }
    
    @objc func applyRecurringForce() {
